...
# The code for tranforming both the query and the cacm_stem.txt file is provided.
# Make sure to run the query file and corpus through these programs before
# passing them here.

# The program requires 2 things :
# 1. The query file which is stemmed
# 2. Location of the corpus (text documents) stemmed

SysName = "BM25-Stemmed-SRA"
# Location of the stemmed version of the corpus
p = "C:\Users\shashi\Documents\GitHub\IRproject\Common\Stemmed_Corpus"
dirs = os.listdir(p)
corpus = {}
relevant = {}
queries = []
...
# Fetch queries from a given file
def get_queries(filename):
    queries1 = {}
    count2 = 1
    with open(filename) as f:
        for line in f:
            queries1[count2] = str(line).strip()
            count2 += 1
    f.close()
...
def main():
    ...
    queries = get_queries("cacm_stem.query")      # Obtain the queries
    for each in corpus:         # Generate the unigram index
        ...
        table[each] = length
    temp = {}
    for qid in range(1, len(queries) + 1): # BM25 scoring for each of the queries
        temp[qid] = (score_doc(queries[qid].split(), table, qid))
    print_results(temp)
